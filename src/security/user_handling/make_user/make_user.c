...
int make_user(char *users_directory, char *password,
   ...
   * .../userdirect/username*/
  if (SUCCESS != make_file_path(path_userfile,
  ...
  if (SUCCESS ==
    ...
    log_error(
        "eror while making new user : user already exists");
    abort();
    return ERROR_REGISTER_USER_EXISTS;
  }

  if (NULL == (file = fopen(path_userfile, "w"))) {
    log_error("eror while making new user : couldnt make "
              "userfile");
