...
int get_salt_nd_hashedpass(const char *users_path,
  ...
  strncat(pathtofile,users_path,str_maxlen);
  strncat(pathtofile,username,str_maxlen);
  ...
  while (fgets(line,hash_maxlen,file)&& tracker++ < 2){
    switch (tracker) {
      case 0:
        strncat((char*)hashed_password,line,hash_maxlen);
        break;
      case 1:
        strncat((char *)salt,line,salt_maxlen);
        break;
    }
  }
  if(2 == tracker){
...
int login(const char *users_path,
  ...
  if(strnlen(input_password,str_maxlen+1) >= str_maxlen  ||
    strnlen(input_username,str_maxlen+1) >= str_maxlen  ||
    strnlen(users_path,str_maxlen+1) >= str_maxlen )
  ...
  if(SUCCESS != user_exists(users_path,input_username,str_maxlen)){
    return ERROR_USER_DOESNT_EXIST;
  }
  ...
  if(SUCCESS != (err = compare_creds((const char *)input_password_hash,
