...
int get_data_into_json(cJSON *json_obj, const char *list[],
  ...
  char **array_credentials =
      malloc(number_of_inf * sizeof(char *));
  ...
  getinfo(list, list_clarifications, number_of_inf, maxlen,
          array_credentials);
  if (SUCCESS != errno) {
    goto free_resources;
  }
  ...
  for (int i = 0; i < number_of_inf; ++i) {
    if (NULL ==
        cJSON_AddStringToObject(json_obj, list[i],
                                array_credentials[i])) {
...
{
  /*the string that will hold the json format of the
   * account*/
  ...
  unsigned char *hex =
      malloc(HEX_SIZE(CIPHER_SIZE(ACCOUNT_MAX_SIZE,
                                  AES_256_BLOCK_SIZE)) *
             sizeof(char));
  if (NULL == hex) {
  ...
  if (NULL == json || NULL == username || NULL == key ||
      NULL == EVP_CBC_FUNC) {
  ...
  ciphersize =
      encrypt(string, strlen((const char *)string), key,
              username, cipher_acc, EVP_CBC_FUNC);

  /*converting to hex for ease of storage*/
  binary_to_hex(cipher_acc,
                CIPHER_SIZE(strlen((const char *)string),
                            AES_256_BLOCK_SIZE),
                hex);
  ...
  cJSON_AddItemToObject(temporay, "cipher hex",
                        json_item_hexacc);
  cJSON_AddItemToObject(temporay, "cipher lengh",
                        json_item_cipherlen);
...
int decrypt_json(cJSON *encrypted_acc_json, cJSON **json,
  ...
  if (NULL ==
      (json_item_hexacc = cJSON_GetObjectItemCaseSensitive(
           encrypted_acc_json, "cipher hex"))) {
    errno = ERROR_JSON_ADDING_ITEM_TO_OBJ;
    goto end;
  }
  if (NULL ==
      (json_item_cipherlen =
           cJSON_GetObjectItemCaseSensitive(
               encrypted_acc_json, "cipher lengh"))) {
    errno = ERROR_JSON_ADDING_ITEM_TO_OBJ;
    goto end;
  }
  if (NULL == json_item_hexacc ||
  ...
  hex_to_binary(json_item_hexacc->valuestring, cipher_acc,
                json_item_cipherlen->valuedouble);
  decrypt(cipher_acc, json_item_cipherlen->valuedouble, key,
          username, string, EVP_CBC_FUNC);
  if (errno != SUCCESS)
    goto end;
...
int modify_item(cJSON *object_to_modify, char *newvalue,
  ...
  /*thi is done to ensure we have enough memory in the item
   * str*/
  free(object_to_modify->valuestring);
  object_to_modify->valuestring =
...
int modify_json(cJSON *jsontomod, const char *item_name,
  ...
  if (NULL == jsontomod || NULL == item_name ||
      NULL == newvalue) {
    errno = ERROR_NULL_VALUE_GIVEN;
    goto free_resources;
  }
  cJSON *itemtomod = cJSON_GetObjectItemCaseSensitive(
      jsontomod, item_name);
