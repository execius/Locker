...
int encrypt(const unsigned char *plaintext,
  ...
  if (LIBSSL_SUCCESS != EVP_EncryptInit_ex(ctx,
                                           EVP_CBC_FUNC(),
                                           NULL, key, iv))
    return (errno = handleErrors());
  ...
  if (LIBSSL_SUCCESS != EVP_EncryptUpdate(ctx, ciphertext,
                                          &len, plaintext,
                                          plaintext_len))
    return (errno = handleErrors());
  ciphertext_len = len;
...
int decrypt(unsigned char *ciphertext, int ciphertext_len,
  ...
  /*
     ...
     * is 128 bits
     */
  if (LIBSSL_SUCCESS != EVP_DecryptInit_ex(ctx,
                                           EVP_CBC_FUNC(),
                                           NULL, key, iv))
   ...
   */
  if (LIBSSL_SUCCESS != EVP_DecryptUpdate(ctx, plaintext,
                                          &len, ciphertext,
                                          ciphertext_len))
    return (errno = handleErrors());
  /*
